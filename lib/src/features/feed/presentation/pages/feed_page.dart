...
class _FeedViewState extends State<FeedView> {
  ...
  // ✅ AdMob banner
  static const String _bannerAdUnitId = 'ca-app-pub-6445825465110921/8356358230';
  BannerAd? _bannerAd;
  ...
  void _initBanner() {
    // Banner solo su Android/iOS (non web)
    _bannerAd = BannerAd(
      ...
      listener: BannerAdListener(
        ...
        onAdFailedToLoad: (ad, error) {
          ad.dispose();
          // Non bloccare app se fallisce: semplicemente niente banner
        },
      ),
    )..load();
  }
  ...
  void _onScroll() {
    if (_scrollController.position.pixels > _scrollController.position.maxScrollExtent * 0.7 &&
      ...
      context.read<FeedBloc>().add(
            FeedFetchedEvent(feedTypeSubview: selectedUserOption[0] ? FeedTypeSubview.post : FeedTypeSubview.comment),
          );
    }
  ...
  Widget build(BuildContext context) {
    ...
    final bannerWidget = (_isBannerLoaded && _bannerAd != null)
        ? SizedBox(
    ...
    return BlocListener<FeedUiCubit, FeedUiState>(
      ...
      child: Scaffold(
        // ✅ Banner fisso in basso (policy-safe)
          ...
          child: BlocConsumer<FeedBloc, FeedState>(
            listenWhen: (previous, current) {
              if (current.excessiveApiCalls) {
                showSnackbar(
                  ...
                  trailingAction: () => navigateToSettingPage(
                    context,
                    LocalSettings.settingsPageFilters,
            ...
            listener: (context, state) {
              if (state.status == FeedStatus.success &&
                  ((selectedUserOption[0] && state.hasReachedPostsEnd == false) ||
                      (selectedUserOption[1] && state.hasReachedCommentsEnd == false))) {
              ...
              if ((state.status == FeedStatus.failure ||
                ...
                context.read<FeedBloc>().add(FeedClearMessageEvent());
              }
            },
                          ...
                          if (state.status != FeedStatus.failureLoadingCommunity && state.status != FeedStatus.failureLoadingUser)
                            SliverToBoxAdapter(
                              child: ((selectedUserOption[0] && state.hasReachedPostsEnd) || (selectedUserOption[1] && state.hasReachedCommentsEnd))
                                  ...
                                  : Container(
                                      height: state.status == FeedStatus.initial ? MediaQuery.of(context).size.height * 0.5 : null,
  ...
  FutureOr<bool> _handleBack(bool stopDefaultButtonEvent, RouteInfo info) async {
    ...
    final desiredFeedListType =
        authBloc.state.siteResponse?.myUser?.localUserView.localUser.defaultListingType ?? feedCubit.state.defaultFeedListType;
    ...
    if (!canPop && (desiredFeedListType != currentFeedListType || communityMode)) {
      final postSortType =
          authBloc.state.siteResponse?.myUser?.localUserView.localUser.defaultSortType ?? feedCubit.state.defaultPostSortType;
